...
function doubleTheFun(doublemebaby) {
  	var OrlyOwl = typeof(doublemebaby*1);
  	if(OrlyOwl == 'number' && isNaN(doublemebaby)!=true) {
      return doublemebaby*2;
    }
  	else {
      return doublemebaby+doublemebaby;
    }
}
...
// #14 Create a function called coderTest that takes in an object that is a person. It looks to see if the person’s name is Jeremy and then changes the person object to have a property called lovesCode with a value of 10.  If their name is Brack set lovesCode to 0.  otherwise set lovesCode to 5.
	function coderTest(obj) {
		for(var key in obj) {
			if(obj.name.toLowerCase() == 'jeremy') {
				obj.lovesCode = 10;
			}
			else if(obj.name.toLowerCase() == 'brack') {
				obj.lovesCode = 0;
			}
			else {
				obj.lovesCode = 5;
...
*/
function outside(temp, humidty, cloudiness) {
	if(temp>80 && humidty>40) {
		return "I'm all sweat";
	}
	else if(temp<40 && cloudiness>60) {
		return "I have icecicles";
	}
	else if(temp>80 && humidty<40 && cloudiness<20) {
		return "I'm literally in the desert";
	}
	else if(temp>80 || humidty>50 || cloudiness>50) {
